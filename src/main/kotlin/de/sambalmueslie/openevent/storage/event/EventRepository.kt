...
interface EventRepository : DataObjectRepository<Long, EventData> {
    ...
    @Query(
        value = "select e.* from event e WHERE (e.owner_id = :ownerId and e.published = true) AND DATE(start) >= CURRENT_DATE ORDER BY e.start",
        countQuery = "select e.* from event e WHERE (e.owner_id = :ownerId and e.published = true) AND DATE(start) >= CURRENT_DATE"
